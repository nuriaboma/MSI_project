...
| **Trajectory**      | `.xtc` or `.trr` | **Analysis of movement and structure.** This is the key file. It is used to calculate: the Mean Squared Deviation (RMSD), residue fluctuations (RMSF), distances between atoms, radius of gyration, formation or breaking of hydrogen bonds, and visualization of molecular dynamics. If it is `.trr`, it also allows analyzing velocities and forces. |

| **Energy**          | `.edr`          | **Thermodynamic analysis.** It is used to monitor the stability and quality of the simulation. It allows graphing the evolution of potential energy, total energy, temperature and pressure, verifying that the system remains in equilibrium and that energy conservation is adequate.                                                                          |

| **Log**         | `.log`          | **Diagnostics and performance.** This file is crucial to verify that the simulation ran correctly. It contains details about the initial parameters, the end time, the calculation speed (CPU/GPU performance), and a summary of energy conservation statistics.                                                                           |

| **Final structure** | `.gro`          | **Starting point for analysis or future simulations.** Represents the final configuration of the system at the end of the simulated time and can be used as the initial structure for an extension of the simulation or for static structural analysis.                                                                                                               |

| **Checkpoint** | `.cpt`          | **Restart and continuation.** This binary file allows the user to stop the simulation and restart it exactly from the same point at another time or on another machine. It is essential for very long simulations or to recover from interruptions (for example, system failures).                                                                                       |

| **State**           | `.ndx`          | **Selection of atom groups for analysis.** Defines lists of atoms (groups) that are of interest for analysis. It is used as an input file for many GROMACS post-processing tools.                                                                                                                                                                   |
